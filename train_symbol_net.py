...
from network import eco_full_symbol
...
def get_schedule(epochs, train_data, batch_size):
    ...
    logging.info(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + " Learning rate drops after iterations: {}".format(steps_iterations))
...
def split_image_dataset(train_path, val_path, anno_file, val_size=0.2):
    ...
    if not os.path.exists(anno_file):
        warnings.warn(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' Annotations file %s is not exists.' % anno_file, stacklevel=3)
...
def parse_args():
    ...
    parser.add_argument('--gpus', dest='gpus', default='0,3', type=str,
